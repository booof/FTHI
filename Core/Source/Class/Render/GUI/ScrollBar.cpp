...
ScrollBar::ScrollBar(float xPos, float yPos, float width, float height, float size, float per)
{
	...
	else
	{
		...
		BarOffset = height_difference * percent;
	}
...
ScrollBar::ScrollBar() { BarPosX = NULL; BarPosY = NULL; MinPos = NULL; MaxPos = NULL; BarWidth = NULL; BarHeight = NULL; BackgroundVAO = NULL; BackgroundVBO = NULL; ScrollVAO = NULL; ScrollVBO = NULL; }

void ScrollBar::Move(float newX, float newY)
{
	// Calculate New Positon Values
	BarPosX = newX;
	BarPosY = (BarPosY - DefaultY) + newY; // New Y pos is the New Y Position Plus the Offset the Bar is Currently at
	MaxPos -= DefaultY - newY;
	MinPos -= DefaultY - newY;
	DefaultY = newY;
...
bool ScrollBar::TestColloisions()
{
...
float ScrollBar::Scroll(float newY)
{
	// Set New Y Position
	BarPosY = newY;
