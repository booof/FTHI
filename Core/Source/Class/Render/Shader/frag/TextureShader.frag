// Fragment Shader for Creating Textured Objects Affected by Light
#version 450 core
...
layout (std140, binding = 1) buffer Directional_Lights
{
...
vec4 CalculatePointLight(Point light, vec4 normal, vec4 fragpPos, vec4 viewDirection, vec4 color_ambient, vec4 color_diffuse, vec4 color_specular);

// Uniform Buffer For Point Lights
layout (std140, binding = 2) buffer Point_Lights
{
	int point_count;
	Point point_lights[];
};
...
vec4 CalculateSpotLight(Spot light, vec4 normal, vec4 fragpPos, vec4 viewDirection, vec4 color_ambient, vec4 color_diffuse, vec4 color_specular);

// Uniform Buffer For Spot Lights
layout (std140, binding = 3) buffer Spot_Lights
{
	int spot_count;
	Spot spot_lights[];
};
...
void main()
{
	...
	// Bools for Texture
	bool animated = (material.texture_layer > 0);
	...
	float alpha = Alpha * color_diffuse.w;
